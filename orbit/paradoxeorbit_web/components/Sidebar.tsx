...
const SidebarComponent = ({ isCollapsed, onToggle, onSearchClick }: SidebarProps) => {
    ...
    const bottomItems = useMemo(
        () => [
            ...
            {
                href: 'https://www.dropbox.com/scl/fi/esk4h8z45sryvbremy57v/Paradoxe_latest.dmg?rlkey=92y535bz6p6gov6vd17x6q53b&st=9kl0annj&dl=1',
                icon: '/download.svg',
                text: 'Download Paradoxe Camera',
                ariaLabel: 'Download Paradoxe Camera (new window)',
            },
            {
                ...
                icon: '/download.svg',
                text: 'Download Paradoxe Orbit',
                ariaLabel: 'Download Paradoxe Orbit (new window)',
            },
        ],
        []
    );
    ...
    return (
        ...
        >
            <header className={`group relative h-6 flex shrink-0 items-center justify-between`}>
                {isCollapsed ? (
                    <Link href="https://paradoxe.com" target="_blank" rel="noopener noreferrer" className="flex items-center">
                ...
                {!isCollapsed && hasApiKey !== null && (
                    <div className="px-2.5 py-2 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full ${hasApiKey ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                            {hasApiKey ? 'Local running' : 'Paradoxe Free System'}
                        </span>
                    </div>
                )}
