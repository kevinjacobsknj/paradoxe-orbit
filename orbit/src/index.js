...
function setupProtocolHandling() {
    ...
    try {
        if (!app.isDefaultProtocolClient('paradoxeorbit')) {
            const success = app.setAsDefaultProtocolClient('paradoxeorbit');
            if (success) {
                console.log('[Protocol] Successfully set as default protocol client for paradoxeorbit://');
            } else {
                console.warn('[Protocol] Failed to set as default protocol client - this may affect deep linking');
            }
        } else {
            console.log('[Protocol] Already registered as default protocol client for paradoxeorbit://');
        }
    } catch (error) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        for (const arg of commandLine) {
            if (arg && typeof arg === 'string' && arg.startsWith('paradoxeorbit://')) {
    ...
    app.on('open-url', (event, url) => {
        ...
        if (!url || !url.startsWith('paradoxeorbit://')) {
...
async function handleCustomUrl(url) {
    try {
        ...
        if (!url || typeof url !== 'string' || !url.startsWith('paradoxeorbit://')) {
...
async function handleFirebaseAuthCallback(params) {
    ...
    try {
        const functionUrl = 'https://us-west1-paradoxe-3651a.cloudfunctions.net/paradoxeOrbitAuthCallback';
...
async function startWebStack() {
  ...
  process.env.PARADOXE_ORBIT_API_PORT = apiPort.toString();
  process.env.PARADOXE_ORBIT_API_URL = `http://localhost:${apiPort}`;
  process.env.PARADOXE_ORBIT_WEB_PORT = frontendPort.toString();
  process.env.PARADOXE_ORBIT_WEB_URL = `http://localhost:${frontendPort}`;

  console.log(`🌍 Environment variables set:`, {
    PARADOXE_ORBIT_API_URL: process.env.PARADOXE_ORBIT_API_URL,
    PARADOXE_ORBIT_WEB_URL: process.env.PARADOXE_ORBIT_WEB_URL
  });

  const createBackendApp = require('../paradoxeorbit_web/backend_node');
  const nodeApi = createBackendApp(eventBridge);

  const staticDir = app.isPackaged
    ? path.join(process.resourcesPath, 'out')
    : path.join(__dirname, '..', 'paradoxeorbit_web', 'out');
  ...
  if (!fs.existsSync(staticDir)) {
    ...
    console.error(`Please run 'npm run build' inside the 'paradoxeorbit_web' directory first.`);
    console.error(`============================================================`);
    app.quit();
    return;
  }
    ...
    try {
        ...
        autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName, date, url) => {
            ...
            dialog.showMessageBox({
                ...
                message: `A new version of ParadoxeOrbit (${releaseName}) has been downloaded. It will be installed the next time you launch the application.`,
