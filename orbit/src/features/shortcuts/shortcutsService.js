...
class ShortcutsService {
    ...
    async registerShortcuts(registerOnlyToggleVisibility = false) {
        ...
        // Push-to-talk functionality using F1 key (Fn may not be registerable)
        try {
            const registered = globalShortcut.register('F1', () => {
                console.log('[Push-to-talk] F1 key pressed - starting recording');
                if (header && !header.isDestroyed()) {
                    console.log('[Push-to-talk] Sending push-to-talk-toggle to header');
                    header.webContents.send('push-to-talk-toggle');
                } else {
                    console.log('[Push-to-talk] Header window not available');
                }
            });
            console.log('[Shortcuts] F1 key push-to-talk shortcut registered:', registered);
        } catch (error) {
            console.error('[Shortcuts] Failed to register F1 key shortcut:', error);
        }
        ...
        // Test IPC message to verify communication (send after delay to ensure MainHeader is loaded)
        if (header && !header.isDestroyed()) {
            setTimeout(() => {
                console.log('[Shortcuts] Sending delayed test IPC message to header');
                header.webContents.send('test-ipc-message', { test: 'Hello from shortcuts service!' });
            }, 2000);
        }
    }
