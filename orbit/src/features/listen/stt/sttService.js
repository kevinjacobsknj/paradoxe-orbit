...
class SttService {
    ...
    async initializeSttSessions(language = 'en') {
        ...
        const sttOptions = {
            ...
            language: effectiveLanguage,
            usePortkey: this.modelInfo.provider === 'openai-orbit',
            portkeyVirtualKey: this.modelInfo.provider === 'openai-orbit' ? this.modelInfo.apiKey : undefined,
        };
    ...
    async sendMicAudioContent(data, mimeType) {
        // If push-to-talk session is active, send to it instead
        if (this.pushToTalkSession) {
            return await this.sendPushToTalkAudio(data, mimeType);
        }
    ...
    async closeSessions() {
        ...
        this.pushToTalkSession = null;
        this.pushToTalkBuffer = '';
    }

    // Push-to-talk functionality using Deepgram
    async startPushToTalkSession() {
        if (this.pushToTalkSession) {
            console.log('[Push-to-talk] Session already active');
            return { success: false, error: 'Session already active' };
        }

        try {
            console.log('[Push-to-talk] Initializing Deepgram session...');
            
            // Get Deepgram API key from environment variable
            const deepgramKey = process.env.DEEPGRAM_API_KEY;

            if (!deepgramKey) {
                throw new Error('Deepgram API key not configured. Please add DEEPGRAM_API_KEY to your .env file.');
            }
            ...
            const handlePushToTalkMessage = (message) => {
                console.log('[Push-to-talk] Received message:', message);
                ...
                if (isFinal) {
                    this.pushToTalkBuffer += (this.pushToTalkBuffer ? ' ' : '') + text.trim();
                    console.log('[Push-to-talk] Final transcript:', this.pushToTalkBuffer);
                }
            };

            const pushToTalkConfig = {
                language: 'en',
                callbacks: {
                    onmessage: handlePushToTalkMessage,
                    onerror: error => console.error('[Push-to-talk] STT error:', error.message),
                    onclose: event => console.log('[Push-to-talk] STT closed:', event.reason),
                },
            };
            ...
            const sttOptions = {
                ...
                language: 'en',
                callbacks: pushToTalkConfig.callbacks,
                sessionType: 'pushToTalk'
            };

            const { createSTT } = require('../../common/ai/factory');
            this.pushToTalkSession = await createSTT('deepgram', sttOptions);
            this.pushToTalkBuffer = '';

            console.log('[Push-to-talk] Deepgram session initialized successfully');
            return { success: true };

        } catch (error) {
            console.error('[Push-to-talk] Error starting session:', error);
            this.pushToTalkSession = null;
            return { success: false, error: error.message };
        }
    }

    async stopPushToTalkSession() {
        if (!this.pushToTalkSession) {
            console.log('[Push-to-talk] No active session to stop');
            return { success: false, error: 'No active session' };
        }

        try {
            console.log('[Push-to-talk] Stopping session...');
            
            await this.pushToTalkSession.close();
            this.pushToTalkSession = null;

            const transcription = this.pushToTalkBuffer.trim();
            this.pushToTalkBuffer = '';

            console.log('[Push-to-talk] Session stopped, transcription:', transcription);
            ...
            console.error('[Push-to-talk] Error stopping session:', error);
            this.pushToTalkSession = null;
            this.pushToTalkBuffer = '';
            return { success: false, error: error.message };
        }
    ...
    // Method to send mic audio to push-to-talk session
    async sendPushToTalkAudio(data, mimeType) {
        if (!this.pushToTalkSession) {
            throw new Error('Push-to-talk session not active');
        }

        try {
            // Convert to buffer for Deepgram
            const payload = Buffer.from(data, 'base64');
            await this.pushToTalkSession.sendRealtimeInput(payload);
        } catch (error) {
            console.error('[Push-to-talk] Error sending audio:', error);
            throw error;
        }
    }
}
