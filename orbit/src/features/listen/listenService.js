...
class ListenService {
    constructor() {
        // Re-enabled ListenService for push-to-talk functionality
    ...
    async handleListenRequest(listenButtonText) {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] handleListenRequest skipped - service disabled');
            return;
        }
        const { windowPool } = require('../../window/windowManager');
        const listenWindow = windowPool.get('listen');
    ...
    async initializeSession(language = 'en') {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] initializeSession skipped - service disabled');
            return false;
        }
    ...
    async sendMicAudioContent(data, mimeType) {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] sendMicAudioContent skipped - service disabled');
            return { success: false, error: 'Listen service temporarily disabled' };
        }
    ...
    async startMacOSAudioCapture() {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] startMacOSAudioCapture skipped - service disabled');
            throw new Error('Listen service temporarily disabled for debugging');
        }
    ...
    async stopMacOSAudioCapture() {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] stopMacOSAudioCapture skipped - service disabled');
            return;
        }
    ...
    async closeSession() {
        if (this.temporarilyDisabled) {
            console.log('[ListenService] closeSession skipped - service disabled');
            return { success: true };
        }
    ...
    handleStartPushToTalk = this._createHandler(
        async () => {
            if (this.temporarilyDisabled) {
                console.log('[ListenService] handleStartPushToTalk skipped - service disabled');
                return { success: false, error: 'Listen service temporarily disabled for debugging' };
            }
            console.log('[Push-to-talk] Starting Deepgram push-to-talk session...');
            return await this.sttService.startPushToTalkSession();
        },
        'Push-to-talk session started.',
        'Error starting push-to-talk session:'
    );

    handleStopPushToTalk = this._createHandler(
        async () => {
            if (this.temporarilyDisabled) {
                console.log('[ListenService] handleStopPushToTalk skipped - service disabled');
                return { success: true }; // Return success to avoid errors in UI
            }
            console.log('[Push-to-talk] Stopping push-to-talk session...');
            return await this.sttService.stopPushToTalkSession();
        },
        null,
        'Error stopping push-to-talk session:'
    );
}
