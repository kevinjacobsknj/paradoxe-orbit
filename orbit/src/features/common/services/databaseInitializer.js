...
class DatabaseInitializer {
    constructor() {
        ...
        //   macOS: ~/Library/Application Support/Orbit/paradoxeorbit.db
        //   Windows: %APPDATA%\Orbit\paradoxeorbit.db
        this.dbPath = path.join(userDataPath, 'paradoxeorbit.db');
        this.dataDir = userDataPath;

        // The original DB path (read-only location in the package)
        this.sourceDbPath = app.isPackaged
            ? path.join(process.resourcesPath, 'data', 'paradoxeorbit.db')
            : path.join(app.getAppPath(), 'data', 'paradoxeorbit.db');
    }
