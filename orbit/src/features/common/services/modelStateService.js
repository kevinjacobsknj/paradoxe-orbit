...
class ModelStateService extends EventEmitter {
    constructor() {
        ...
        this.store = new Store({ name: 'paradoxe-orbit-model-state' });
    }
    ...
    async initialize() {
        ...
        await this._loadEnvironmentApiKeys();
    ...
    async _loadEnvironmentApiKeys() {
        console.log('[ModelStateService] Loading API keys from environment variables...');
        ...
        let keysLoaded = 0;
        for (const [provider, envKey] of Object.entries(envKeyMapping)) {
            if (envKey && PROVIDERS[provider]) {
                try {
                    // Check if key already exists to avoid overwriting user settings
                    const existingSettings = await providerSettingsRepository.getByProvider(provider);
                    if (!existingSettings?.api_key || existingSettings.api_key === 'local') {
                        console.log(`[ModelStateService] Loading ${provider} API key from environment`);
                        await this.setApiKey(provider, envKey);
                        keysLoaded++;
                    } else {
                        console.log(`[ModelStateService] ${provider} API key already exists in database, skipping environment load`);
                    }
                } catch (error) {
                    console.warn(`[ModelStateService] Failed to load ${provider} API key from environment:`, error.message);
                }
            }
        }
        
        if (keysLoaded > 0) {
            console.log(`[ModelStateService] Loaded ${keysLoaded} API key(s) from environment variables`);
        } else {
            console.log('[ModelStateService] No new API keys loaded from environment');
        }
    }
    ...
    async setFirebaseVirtualKey(virtualKey) {
        ...
        // 키를 설정하기 전에, 이전에 openai-orbit 키가 있었는지 확인합니다.
        const previousSettings = await providerSettingsRepository.getByProvider('openai-orbit');
        const wasPreviouslyConfigured = !!previousSettings?.api_key;

        // 항상 새로운 가상 키로 업데이트합니다.
        await this.setApiKey('openai-orbit', virtualKey);

        if (virtualKey) {
            // 이전에 설정된 적이 없는 경우 (최초 로그인)에만 모델을 강제로 변경합니다.
            if (!wasPreviouslyConfigured) {
                console.log('[ModelStateService] First-time setup for openai-orbit, setting default models.');
                const llmModel = PROVIDERS['openai-orbit']?.llmModels[0];
                const sttModel = PROVIDERS['openai-orbit']?.sttModels[0];
                if (llmModel) await this.setSelectedModel('llm', llmModel.id);
                if (sttModel) await this.setSelectedModel('stt', sttModel.id);
            } else {
                console.log('[ModelStateService] openai-orbit key updated, but respecting user\'s existing model selection.');
            }
        } else {
            // 로그아웃 시, 현재 활성화된 모델이 openai-orbit인 경우에만 다른 모델로 전환합니다.
            ...
            if (llmProvider === 'openai-orbit') typesToReselect.push('llm');
            if (sttProvider === 'openai-orbit') typesToReselect.push('stt');
    ...
    async setApiKey(provider, key) {
        ...
        // 'openai-orbit'는 자체 인증 키를 사용하므로 유효성 검사를 건너뜁니다.
        if (provider !== 'openai-orbit') {
