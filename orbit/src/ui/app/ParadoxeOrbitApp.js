...
export class ParadoxeOrbitApp extends LitElement {
    static styles = css`
        :host {
            display: block;
            width: 100%;
    ...
    connectedCallback() {
        ...
        if (window.api) {
            window.api.paradoxeOrbitApp.onClickThroughToggled((_, isEnabled) => {
    ...
    disconnectedCallback() {
        ...
        if (window.api) {
            window.api.paradoxeOrbitApp.removeAllClickThroughListeners();
        }
    }
...
customElements.define('paradoxe-orbit-app', ParadoxeOrbitApp);
