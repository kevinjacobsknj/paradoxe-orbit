...
class HeaderTransitionManager {
    constructor() {
        ...
        this.ensureHeader = (type) => {
            ...
            if (type === 'welcome') {
                ...
                console.log('[HeaderController] 🚀 Creating main-header element...');
                this.mainHeader = document.createElement('main-header');
                console.log('[HeaderController] ✅ MainHeader element created:', this.mainHeader);
                console.log('[HeaderController] 🔍 MainHeader constructor called?');
                this.headerContainer.appendChild(this.mainHeader);
                console.log('[HeaderController] ✅ MainHeader element appended to container');
                this.mainHeader.startSlideInAnimation?.();
                console.log('[HeaderController] ✅ MainHeader slide-in animation started');
            }
    ...
    async _resizeForMain() {
        ...
        console.log('[HeaderController] _resizeForMain: Resizing window to 500x47');
        return window.api.headerController.resizeHeaderWindow({ width: 500, height: 47 }).catch(() => {});
    }

    async _resizeForApiKey(height = 370) {
        if (!window.api) return;
        console.log(`[HeaderController] _resizeForApiKey: Resizing window to 500x${height}`);
        return window.api.headerController.resizeHeaderWindow({ width: 500, height: height }).catch(() => {});
    }

    async _resizeForPermissionHeader(height) {
        if (!window.api) return;
        const finalHeight = height || 220;
        return window.api.headerController.resizeHeaderWindow({ width: 500, height: finalHeight })
            .catch(() => {});
    }

    async _resizeForWelcome() {
        if (!window.api) return;
        console.log('[HeaderController] _resizeForWelcome: Resizing window to 500x370');
        return window.api.headerController.resizeHeaderWindow({ width: 500, height: 364 })
            .catch(() => {});
    }
