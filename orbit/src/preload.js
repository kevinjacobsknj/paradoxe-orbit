...
contextBridge.exposeInMainWorld('api', {
  ...
  common: {
    ...
    // User state listener (used by multiple components)
      ...
      // Push-to-talk global shortcut listener and general IPC message handling
      onMessage: (channel, callback) => {
        console.log(`[Preload] Setting up IPC listener for channel: ${channel}`);
        const wrappedCallback = (event, ...args) => {
          console.log(`[Preload] Received IPC message on channel: ${channel}`, args);
          callback(event, ...args);
        };
        ipcRenderer.on(channel, wrappedCallback);
        return wrappedCallback;
      },
      removeMessage: (channel, callback) => {
  ...
  mainHeader: {
    ...
    sendAskMessage: (message) => ipcRenderer.invoke('ask:sendQuestionFromHeader', message),
  ...
  // src/ui/app/ParadoxeOrbitApp.js
  paradoxeOrbitApp: {
    // Listeners
  ...
  ask: {
    ...
    sendQuestion: (text) => ipcRenderer.invoke('ask:sendQuestion', text),
    closeAskWindow: () => ipcRenderer.invoke('ask:closeAskWindow'),
    
    // Listeners needed by MainHeader
    onAskStateUpdate: (callback) => ipcRenderer.on('ask:stateUpdate', callback),
    removeOnAskStateUpdate: (callback) => ipcRenderer.removeListener('ask:stateUpdate', callback),
    ...
    onResponseComplete: (callback) => ipcRenderer.on('ask-response-complete', callback),
    removeOnResponseComplete: (callback) => ipcRenderer.removeListener('ask-response-complete', callback)
  },
  
  // Add missing microphone permission function
  requestMicrophonePermission: () => ipcRenderer.invoke('request-microphone-permission'),
  ...
  listenCapture: {
    ...
    startPushToTalkSession: () => ipcRenderer.invoke('start-push-to-talk-session'),
    stopPushToTalkSession: () => ipcRenderer.invoke('stop-push-to-talk-session'),
