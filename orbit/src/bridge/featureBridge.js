...
module.exports = {
  ...
  initialize() {
    ...
    ipcMain.handle('get-web-url', () => process.env.paradoxeorbit_WEB_URL || 'http://localhost:3000');
    ...
    ipcMain.handle('ask:sendQuestionFromHeader', async (event, userPrompt) => await askService.sendMessage(userPrompt));
    ...
    ipcMain.handle('start-push-to-talk-session', async () => await listenService.handleStartPushToTalk());
    ipcMain.handle('stop-push-to-talk-session', async () => await listenService.handleStopPushToTalk());
